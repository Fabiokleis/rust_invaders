use crate::{NUM_COLS, NUM_ROWS, timer::Timer, frame::Drawable};
...
impl Drawable for Invaders {
    fn draw(&self, frame: &mut Frame) {
        for inv in self.army.iter() {
            frame[inv.x][inv.y] = if self.move_timer.time_left.as_secs_f32() / self.move_timer.duration.as_secs_f32() > 0.5 { "x" } else { "+" };
        }
    }
}
